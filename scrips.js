...
// get random item from the array and construct thr mystery
...
let guess = document.querySelector('button');

guess.addEventListener('click', () => {
  console.log(mistery.suspect.name, mistery.weapon.name, mistery.room.name);
  ...
  if (
    guessSuspect === mistery.suspect.name &&
    guessWeapon === mistery.weapon.name &&
    guessRoom === mistery.room.name
  ) {
    alert('You guessed right! You win');
  } else {
    alert('You guessed wrong! You lose!');
    alert(
      `The answer is ${mistery.suspect.name} killed Mr. Boddy in the ${mistery.room.name} with the ${mistery.weapon.name}`
    );
  }
  document.body.innerHTML += `<div><img width= 400px' heigth=' auto' src="${mistery.suspect.image}" alt="misteryMurder " /></div>`;
});

// showArrays(suspectsArray);
// showArrays(weaponsArray);
// showArrays(roomArray);
